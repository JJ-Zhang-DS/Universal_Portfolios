...
For two assets, let (bin[0,1]) be the allocation to asset A; (1-b) goes to asset B. Each value of (b) defines a **constant-rebalanced portfolio (CRP)**.
...
[
S_t(b)=S_{t-1}(b)left[b x_{t,A}+(1-b)x_{t,B}ight],
qquad S_0(b)=1.
]
...
Cover's original two-asset formulation averages over every (bin[0,1]), not only a finite grid:

[
hat b_t=
rac{int_0^1 b,S_{t-1}(b),db}
{int_0^1 S_{t-1}(b),db}.
]
...
[
hat b_1=
rac{int_0^1 b,db}{int_0^1 1,db}
=rac12.
]

Thus two-asset UP starts at 50/50 and adapts as the candidate CRPs accumulate different wealth.
